    ...
    , objectionModeEnabled(false) // 默认关闭异议模式
{
    ui.setupUi(this);
    setWindowTitle("小冥");
    // 修改点1：禁用自动缩放，改为手动高质量缩放
    ui.petLabel->setScaledContents(false);  // 关闭自动缩放
    ui.petLabel->resize(100 * scale, 100 * scale);
    loadFormSetting();      // 新增形态加载 ← 关键添加位置
    // 初始化GIF加载
    // 动态生成资源路径（关键修改）
    QString basePath = QString(":/gifs%1/walk/0.gif").arg(isForm2 ? "2" : "1");

    // 初始化GIF加载
    petMovie = new QMovie(basePath);

    connect(petMovie, &QMovie::frameChanged, this, [=](int frame) {
        if (frame >= 0) {
            // 获取当前帧并高质量缩放
            QImage img = petMovie->currentImage();
            img = img.scaled(ui.petLabel->size(),
                Qt::KeepAspectRatio,
                Qt::SmoothTransformation); // 关键：启用平滑缩放
            img = sharpenImage(img); // 应用锐化
            ui.petLabel->setPixmap(QPixmap::fromImage(img));
        }
        });
    ...
    move(desktopRect.x() - 20 * scale, desktopRect.y() + desktopRect.height() - 30 * scale);
    ...
    currentInstance = this;  // 关键！将静态指针指向当前实例
    // 连接右键菜单
    ...
    connect(petMovie, &QMovie::frameChanged, [=](int frame) {
        ...
        if (frame == petMovie->frameCount() - 1) { // 若此时为最后一帧
            ...
            case BRIEFCASEIN:
            {
                petMovie->setFileName(QString(":gifs%1/special/think0.gif").arg(isForm2 ? 2 : 1));
            ...
            case BRIEFCASESTOP:
            {
                petMovie->setFileName(QString(":gifs%1/stand/0.gif").arg(isForm2 ? 2 : 1));
...
// 静态指针初始化（类外定义）
MilesEdgeworth* MilesEdgeworth::currentInstance = nullptr;
void MilesEdgeworth::saveFormSetting(bool form) {
    QSettings settings("MilesEdgeworth", "DesktopPet"); // 保持与语言相同的作用域
    settings.setValue("form", form);
}

// 加载形态设置
void MilesEdgeworth::loadFormSetting() {
    QSettings settings("MilesEdgeworth", "DesktopPet");
    isForm2 = settings.value("form", false).toBool(); // 默认形态1
}
...
void MilesEdgeworth::applyScale(double scaleValue, QAction* action)
{
    ...
    // 强制刷新当前帧（优化卡顿）
    if (petMovie->currentFrameNumber() >= 0) {
        ui.petLabel->setPixmap(      // [+3行]
            QPixmap::fromImage(
                sharpenImage(petMovie->currentImage().scaled(newSize, Qt::KeepAspectRatio, Qt::SmoothTransformation))
            ));
    }
    update();
    saveSettings();
    ...
    if (event->buttons() & Qt::LeftButton)
    {
        ...
        if (type != MilesEdgeworth::SLEEP && type != MilesEdgeworth::SLEEPING
            ...
            if (shakeCount == 5 && shakeTimer->isActive()) {
                ...
                petMovie->setSpeed(110);
                petMovie->stop();
                specifyChangeGif(QString(":gifs%1/special/crouch%2.gif").arg(isForm2 ? 2 : 1).arg(direction % 2),
...
void MilesEdgeworth::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    if (type != MilesEdgeworth::SLEEP && type != MilesEdgeworth::SLEEPING&& type != MilesEdgeworth::SLEEPING) {
        ...
        if (type == MilesEdgeworth::CROUCH) {
            petMovie->stop();
            specifyChangeGif(QString(":gifs%1/special/beatb%2.gif").arg(isForm2 ? 2 : 1).arg(direction),
...
void MilesEdgeworth::createContextMenu()
{
    ...
    actionMute->setCheckable(true);  // 设置action可选中
    menu->addSeparator();
    actionChange = menu->addAction("切换形态");
    actionMute->setCheckable(true);  // 设置action可选中
    actionWhip = menu->addAction("闪电五连鞭");
    actionMute->setCheckable(true);  // 设置action可选中
    menu->addSeparator();
    ...
    //切换形态
    connect(actionChange, &QAction::triggered, [=](bool checked) {

        isForm2 = !isForm2;
        saveFormSetting(isForm2);
        // 非站立也非睡觉时, 转为站立动画
        petMovie->stop();
        specifyChangeGif(QString(":/gifs%1/stand/%2.gif").arg(isForm2 ? 2 : 1).arg(direction % 2),
            MilesEdgeworth::STAND, direction % 2);
        update();
        });
    //闪电五连
    connect(actionWhip, &QAction::triggered, [=](bool checked) {
        petMovie->stop();
        int playCount = 5;
        // 使用QPointer管理定时器
        QPointer<QTimer> timer = new QTimer();
        // 使用QSharedPointer管理计数状态
        auto currentCount = QSharedPointer<int>::create(0);
        petMovie->setSpeed(560);
        ...
        QObject::connect(timer, &QTimer::timeout, [=]() {
            ...
            petMovie->setSpeed(560);
            petMovie->stop();
            specifyChangeGif(QString(":/gifs%1/special/beata%2.gif").arg(isForm2 ? 2 : 1).arg(direction),
                MilesEdgeworth::ONCE, direction);
            playComboSound();
    ...
    connect(actionTurn, &QAction::triggered, [=](bool checked) {
        ...
        if (type == MilesEdgeworth::RUN) {
            petMovie->stop();
            // 使用跑步专用的转向动画
            specifyChangeGif(QString(":/gifs%1/special/think%2.gif").arg(isForm2 ? 2 : 1).arg(direction),
                MilesEdgeworth::ONCE, newDirection);
            // 转向后继续跑步
            petMovie->stop();
            specifyChangeGif(QString(":/gifs%1/run/%2.gif").arg(isForm2 ? 2 : 1).arg(newDirection),
            ...
            // 转向后继续走路
            petMovie->stop();
            specifyChangeGif(QString(":/gifs%1/walk/%2.gif").arg(isForm2 ? 2 : 1).arg(newDirection),
...
void MilesEdgeworth::runMove()
{
    ...
    {
        // 向右走
        delta = (QPointF(unit, 0) * scale * 0.2).toPoint();
        break;
    }
    case 1:
    {
    ...
    {
        // 向右上走
        delta = (QPointF(unit, -unit) * scale * 0.2).toPoint();
        break;
    }
    case 3:
    {
    ...
    {
        // 向上走
        delta = (QPointF(0, -unit) * scale * 0.2).toPoint();
        break;
    }
    case 7:
    {
...
void MilesEdgeworth::walkMove()
{
    ...
    {
        // 向右走
        delta = (QPointF(unit, 0) * scale * 0.15).toPoint();
        break;
    }
    case 1:
    {
    ...
    {
        // 向右上走
        delta = (QPointF(unit, -unit) * scale * 0.15).toPoint();
        break;
    }
    case 3:
    {
    ...
    {
        // 向上走
        delta = (QPointF(0, -unit) * scale * 0.15).toPoint();
        break;
    }
    case 7:
    {
...
void MilesEdgeworth::doubleClickEvent()
{
    ...
    QString filename;
     ...
     if (type == MilesEdgeworth::SLEEPING) {
        ...
        if (randnum < 0.25) {       // 挨打1
            petMovie->stop();
            filename = QString(":/gifs%1/special/beata%2.gif").arg(isForm2 ? 2 : 1).arg(direction % 2);
            // 播放whip音效
            playComboSound();
        }
            ...
            petMovie->stop();
            if (language == 0) {
                filename = QString(":/gifs%1/objection/objectiona%2.gif").arg(isForm2 ? 2 : 1).arg(direction % 2);
            }
            else if (language == 1) {
                filename = QString(":/gifs%1/objection/objectionb%2.gif").arg(isForm2 ? 2 : 1).arg(direction % 2);
            }
            else { // language == 2
                filename = QString(":/gifs%1/objection/objectionc%2.gif").arg(isForm2 ? 2 : 1).arg(direction % 2);
            }
        ...
        else if (randnum < 0.8) {       // 挨打2
            petMovie->stop();
            filename = QString(":/gifs%1/special/beatb%2.gif").arg(isForm2 ? 2 : 1).arg(direction % 2);
            // 播放whip音效
            playComboSound();
        }
        else {                      //挨打3
            petMovie->stop();
            filename = QString(":/gifs%1/special/beatc%2.gif").arg(isForm2 ? 2 : 1).arg(direction % 2);
            // 播放whip音效
        }
    ...
    if (type == MilesEdgeworth::SLEEPING || type == MilesEdgeworth::SLEEP) {
        ...
        case 0:
        {
            if (y < 23 * scale && y + 1.2 * x > 74 * scale) {
                // 点击脸部, 触发scared动画
                petMovie->stop();
                specifyChangeGif(QString(":/gifs%1/special/angry%2.gif").arg(isForm2 ? 2 : 1).arg(direction),
                    MilesEdgeworth::ONCE, direction);
            }
            else if (y < 23 * scale) {
                ...
                petMovie->stop();
                if (randnum < 0.5) {
                    specifyChangeGif(QString(":/gifs%1/special/bow%2.gif").arg(isForm2 ? 2 : 1).arg(direction),
                        MilesEdgeworth::ONCE, direction);
                }
                else {
                    specifyChangeGif(QString(":/gifs%1/special/beata%2.gif").arg(isForm2 ? 2 : 1).arg(direction),
                ...
                if (y > 46 * scale) {
                    ...
                    specifyChangeGif(QString(":/gifs%1/special/beatc%2.gif").arg(isForm2 ? 2 : 1).arg(direction),
                        MilesEdgeworth::ONCE, direction);
                }
            }
            else {
...
void MilesEdgeworth::handleGlobalDoubleClick(const QPoint& globalPos) {
    ...
    if (objectionModeEnabled) {
        ...
        petMovie->stop();
        specifyChangeGif(QString(":/gifs%1/special/objection%2.gif").arg(isForm2 ? 2 : 1).arg(direction % 2),
            MilesEdgeworth::ONCE, direction);
...
// 音效组合播放实现
void MilesEdgeworth::playComboSound() {
    // 播放whip音效
    QSoundEffect* whipSound = new QSoundEffect(this);
    whipSound->setSource(QUrl::fromLocalFile(":/audios/whip.wav")); // 修正路径
    whipSound->play();
    // 当whip播放完成后播放damage
    QObject::connect(whipSound, &QSoundEffect::playingChanged, this, [this, whipSound]() {
        if (!whipSound->isPlaying()) {
            soundEffect->setSource(QUrl::fromLocalFile(":/audios/damage.wav"));
            soundEffect->play();
            whipSound->deleteLater(); // 清理临时对象
        }
        });
}
